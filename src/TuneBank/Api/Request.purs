...
requestComment baseUrl genre tuneId key credentials =
  H.liftAff do
    res <- requestTheBody $ defaultJsonGetRequest baseUrl (Just credentials) (Comment genre tuneId key.commentId)
    case res of
      Left err ->
        pure $ Left err
      Right json -> do
...
deleteComment baseUrl genre tuneId commentId credentials =
  H.liftAff do
    res <- requestTheBody $ defaultDeleteRequest baseUrl (Just credentials) (Comment genre tuneId commentId)
    pure res
